...
//       <button class="nav-btn icon-btn" onclick="openNotif()" title="შეტყობინებები">🔔</button>
//       <a href="profile.html" class="nav-btn" style="display:flex;align-items:center;gap:6px"><span class="nav-avatar">${i}</span>${user.firstName||user.phoneNumber}</a>
...
//   if(yFrom)params.append('yearFrom',yFrom);
//   if(yTo)params.append('yearTo',yTo);
...
//   await loadCities();
//   try{
//     const r=await fetch(`${API}/api/Car`);
//     if(r.ok){allCars=await r.json();renderCars(allCars);}
//     else throw 0;
//   }catch{
